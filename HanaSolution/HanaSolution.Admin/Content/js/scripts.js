        ...
        if ($.isFunction($.fn.slider)) {
            $(".slider").each(function(i, el) {
                ...
                // Normal Slider
                    ...
                    var $handle1s = $this.find('.ui-slider-handle');
                    //$fill = $('<div class="ui-fill"></div>');

                    $label_1.html((prefix ? prefix : '') + value + (postfix ? postfix : ''));
                    $handle1s.html($label_1);
...
//                        $this.find('span').html(start.format(drp.format) + drp.separator + end.format(drp.format));
        ...
        if ($.isFunction($.fn.select2)) {
            ...
            $("#s2example-2").select2({
                placeholder: 'Chọn hình thức giới hạn',
                allowClear: true
            }).on('select2-open', function() {
                // Adding Custom Scrollbar
                $(this).data('select2').results.addClass('overflow-hidden').perfectScrollbar();
    ...
    ULTRA_SETTINGS.ultraToDoAddTaskWidget = function() {
        ...
        $(".wid-add-task input").on('keypress', function(e) {
            if (e.keyCode === 13) {
                ...
                var msg = $(this).val();
                var msgs = '<li><input type="checkbox" id="task-' + i + '" class="icheck-minimal-white todo-task"><label class="icheck-label form-label" for="task-' + i + '">' + msg + '</label></li>';
                $(this).parent().parent().find(".wid-all-tasks ul").append(msgs);
                $(this).val("");
                $(this).focus();
